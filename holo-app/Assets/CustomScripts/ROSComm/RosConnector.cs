...
{
    public class RosConnector : MonoBehaviour  // Client is our RosConnector
    {
        public int Timeout = 10; // timeout to wait for communication with ROS

        public RosSocket RosSocket { get; private set; }
        public enum Protocols { WebSocketSharp, WebSocketNET, WebSocketUWP };
        public Protocols Protocol;
        
        private string RosBridgeServerUrl = Utils.ROS_SERVER_IP;
        private ManualResetEvent isConnected = new ManualResetEvent(false);

        public void Awake() // on Awake start a new thread
        {
#if WINDOWS_UWP
            ConnectAndWait();
#else
        ...
        private void OnConnected(object sender, EventArgs e) // callback function
        {
            isConnected.Set();
            Debug.Log("Connected to RosBridge: " + RosBridgeServerUrl);
        }

        private void OnClosed(object sender, EventArgs e) // callback function
        {
            isConnected.Reset();
